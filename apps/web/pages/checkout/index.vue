...
const adyenPaymentMethod = ref();
const stripePaymentMethod = ref();
...
const radioModel = ref(`${stripePaymentMethod.value?.id}`);
const selectedProvider = ref(stripePaymentMethod.value?.id);

onMounted(() => {
  if (paymentMethods.value?.length) {
    stripePaymentMethod.value = paymentMethods.value.find(
      (i) => i.code === "stripe",
    );
    adyenPaymentMethod.value = paymentMethods.value.find(
      (i) => i.code === "adyen",
    );
    selectedProvider.value = stripePaymentMethod.value?.id;
    radioModel.value = `${deliveryMethods.value?.[0]?.id}`;
    ...
    <div v-else>
      <div class="lg:grid lg:grid-cols-12 md:gap-x-6">
              ...
              >
                ...
                >
                  <div class="flex gap-2">
                    ...
                    <div>
                      <p>{{ name }}</p>
                      <p class="text-xs text-neutral-500">{{ price }}</p>
                    </div>
                  </div>
                </SfListItem>
              </ul>
        ...
        <LazyCheckoutAdyenPaymentProvider
          v-if="showPaymentModal && adyenPaymentMethod"
          :provider="adyenPaymentMethod"
          :cart="cart"
        ...
        />
        <LazyCheckoutStripePaymentProvider
          v-if="showPaymentModal && stripePaymentMethod"
          :provider="stripePaymentMethod"
          :cart="cart"
          :billing-address="savedBillingAddress"
          @is-payment-ready="($event) => (isPaymentReady = $event)"
          @provider-payment-handler="
            ($event) => (providerPaymentHandler = $event)
          "
