    ...
    <fieldset>
      ...
      <div class="grid gap-4 grid-cols-2">
        <label v-for="method in paymentMethods" :key="method.id" class="relative">
          <input type="radio" name="payment_method" class="peer sr-only" :value="method.id"
            :checked="method.id === activePayment" @change="$emit('update:activePayment', method.id)" />
          <span
            ...
            <div class="flex space-x-2">
              <NuxtImg v-for="img in method.paymentIcons" :key="`${img.id}`"
                :src="`https://vsfdemo15.labs.odoogap.com${img.image}`" :alt="img.name" width="30" height="20" />
            </div>
